...
AcquisitionMin = 15;                          % データ取得時間 [分]
AcquisitionSec = 34;                         % データ取得時間 [秒]
...
% 2.3 RF吸収スペクトルの取得及び表示（スムージング処理前）
...
% 3.1.1 マスク後のRF吸収スペクトルの表示及び吸収線ピークの検出、表示
figure
...
%% RF吸収スペクトルのスムージング処理 (必要であればコメント化解除して使用)
% スムージング処理 (やり過ぎると分解能が下がるので非推奨)
% SmthAbsorption = movmean(abs(SmthAbsorption), smth);                       % RF吸収スペクトルのスムージング処理

% スムージング処理後のRF吸収スペクトルの表示
figure
plot(f3, SmthAbsorption, 'LineWidth', 1);
xlabel('Frequency [MHz]')                                     % x軸ラベル
ylabel('Transmittance [a.u.]')                                % y軸ラベル
...
% グラフの保存 (emf形式)
if Judge == 1 || Judge == 2
    saveas(gcf, fullfile(EmfFolder, [Name, '_3.1.2 Smoothed RF Absorption.emf']), 'emf');
end
...
% グラフの保存 (emf形式)
if Judge == 1 || Judge == 2
    saveas(gcf, fullfile(EmfFolder, [Name, '_3.1.3 Peak Plotted Noise masked RF Absorption.emf']), 'emf');
end
...
% 8.1.2 吸収スペクトルの取得及び表示、HITRANとの比較 (推定値)
...
% 8.2.2 吸収スペクトルの取得及び表示、HITRANとの比較 (算出値)
